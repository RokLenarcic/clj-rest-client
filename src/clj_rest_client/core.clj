...
(defmacro defrest-map [definition {:keys [json-responses jsonify-bodies param-transform val-transform client defaults name-transform json-opts default-method]
                                   :or {json-responses true jsonify-bodies :smart client identity defaults {} name-transform identity default-method :get}}]
  (let [opts-map {:default-method default-method
...
(defmacro defrest
  ...
  `:name-transform` This option specifies function that is used to transform name symbol into fn name. Default `identity`.

  `:param-transform` This option specifies function that is used to transform query parameter names: parameter name (symbol) -> query parameter name (string). Default `identity`.
