...
(defn paged-requests [url do-req req]
  (let [resp (do-req req)]
    (cons (:body resp)
      (when-let [next-link (get-in resp [:links :next :href])]
        (let [next-url (.substring ^String next-link (count url))]
          (lazy-seq (paged-requests url do-req (assoc req :url next-url))))))))

(defn git-client [url user pass]
  (fn [req]
    (if (::iterate-pages req)
      (paged-requests url (do-req-fn url user pass) req)
      ((do-req-fn url user pass) req))))
